...
class TinyVirat(Dataset):
    def __init__(self, cfg, data_split, data_percentage, num_frames, skip_frames, input_size, shuffle=False):
        ...
        self.flow_stats_folder = os.path.join(cfg.flow_stats_folder, data_split)
        self.annotations  = {}
    ...
    def load_all_frames(self, video_path):
        flow_stats_path = video_path.replace(self.data_folder, self.flow_stats_folder).replace('.mp4', '.json')
        with open(flow_stats_path, 'r') as f:
        ...
        frames = torch.from_numpy(np.stack(frames))
        stab_data = np.stack([c for k,c in flow_stats.items() if k not in ['motion_dir_x_std', 'motion_dir_y_std']], axis=1)
        print(flow_stats.keys())
        #stab_data_repeat = 32 // stab_data.shape[1]
        #stab_data_rem = 32 % stab_data.shape[1]
        #stab_data = np.tile(stab_data, (1, stab_data_repeat))
        #stab_data = np.concatenate([stab_data, np.zeros((stab_data.shape[0], stab_data_rem))], axis=1)
        stab_data = torch.from_numpy(stab_data)
        return frames, stab_data
    ...
    def build_consecutive_clips(self, video_path):
        frames, stab_data = self.load_all_frames(video_path)
        if len(frames) % self.num_frames != 0:
            frames = frames[:len(frames) - (len(frames) % self.num_frames)]
            stab_data = stab_data[:len(stab_data) - (len(stab_data) % self.num_frames)]
        clips = torch.stack([self.transform(x) for x in chunks(frames, self.num_frames)])
        stab_data = torch.stack([x for x in chunks(stab_data, self.num_frames)])
        return clips, stab_data
    ...
    def __getitem__(self, index):
        ...
        if self.data_split == 'train':
            clips, stab_data = self.build_consecutive_clips(video_path)
        else:
            clips, stab_data = self.build_consecutive_clips(video_path)
        ...
        if clips.shape[0] < NUM_CLIPS:
            ...
            rem_stab = torch.from_numpy(np.zeros((diff, stab_data.shape[1], stab_data.shape[2]), dtype=int))
            stab_data =  torch.cat((stab_data,rem_stab),dim=0)
        elif clips.shape[0] > NUM_CLIPS:
            clips = clips[:NUM_CLIPS,:,:,:,:]
            stab_data = stab_data[:NUM_CLIPS, ...]
        return clips, stab_data, label #clips: nc x ch x t x H x W
...
#         for i, (clips, d, labels) in enumerate(tqdm(dataloader)):
