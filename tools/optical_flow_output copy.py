...
def cvtflow(flow):
    ...
    hsv[..., 1] = 255
    hsv[..., 0] = ang*180/np.pi/2
    hsv[..., 2] = np.clip(mag * 30, 0, 255).astype(np.uint8)
    bgr = cv.cvtColor(hsv, cv.COLOR_HSV2BGR)
    return bgr
...
for i, (file_name, file_path) in tqdm(enumerate(virat_files)):
    ...
    for i,f in enumerate(frames):
        ...
        p = f if p is None else p
        flow = optical_flow.calc(p, f, None)
        bgr = cvtflow(flow)
        #bgr2 = cvtflow(flow2)

        cv2.imshow('test', cv2.resize(bgr, (512,512)))
        cv2.imshow('test2', cv2.resize(f, (512,512)))
        cv2.imshow('test3', cv2.resize(optical_frames[i], (512,512)))
        cv2.imshow('test4', cv2.resize(bgr + optical_frames[i], (512,512)))
        cv.waitKey(33)
