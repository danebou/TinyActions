...
class ViViT_Pose(nn.Module):
    def __init__(self, spatial_embed_dim=32, sdepth=4, tdepth=4, vid_dim=(20,5,100),
                 num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None, spat_op='cls', tubelet_dim=(3,4),
        ...
        self.Spatial_patch_to_embedding = nn.Linear(c * tt, spatial_embed_dim)
        # nn.Conv3d(c, spatial_embed_dim, self.tubelet_dim[1:],
    ...
    def Spatial_forward_features(self, x, spat_op='cls'):
        ...
        b,nc,ch,np,t,pp = x.shape
        x = rearrange(x, 'b nc ch np t pp -> (b nc np) pp (ch t)') #for spatial transformer, batch size if b*f
        x = self.Spatial_patch_to_embedding(x) #all input spatial tokens, op: (b nc np) x pp x Se
        _,_,Se = x.shape

        class_token=torch.tile(self.spatial_cls_token,(b*nc*np,1,1)) #(B*nc,1,1)
        ...
        cls_token = x[:,-1,:]
        cls_token = torch.reshape(cls_token,(b,nc,np,Se))
        ...
        if spat_op=='cls':
            ...
            x = x[:,:,:]
            x = rearrange(x, '(b nc np) s Se -> (b nc np) Se s')
            x = F.avg_pool1d(x,x.shape[-1],stride=x.shape[-1]) #(b*nc*np) x Se x 1
            x = torch.reshape(x, (b,nc,np,Se))
            return x #b x nc x np x Se

    def Temporal_forward_features(self, x):
        x = rearrange(x, 'b nc np Se -> b (nc np) Se')
        b  = x.shape[0]
    ...
    def forward(self, x):
        #Input x: batch x num_clips x point_channels x number_poses x tublet_time x number_points
