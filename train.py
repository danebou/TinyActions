...
def run():
    ...
    for epoch in range(max_epochs):
        ...
        accuracy = 0.
        cnt = 0.
        for batch_idx, (inputs, targets, frame_pos) in enumerate(tqdm(training_generator)):
            ...
            # Ascent Step
            predictions = model(inputs.float(), frame_pos.float()); #targets = torch.tensor(targets,dtype=torch.long); predictions = torch.tensor(predictions,dtype=torch.long)
